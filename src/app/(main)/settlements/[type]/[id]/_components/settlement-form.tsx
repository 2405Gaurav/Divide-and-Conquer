...
export default function SettlementForm({ entityType, entityData, onSuccess }: SettlementFormProps) {
  ...
  const handleUserSettlement = async (data: SettlementFormData) => {
    ...
    const userData = entityData as UserEntityData;
    ...
    try {
      ...
      toast.error("Failed to record settlement: " + message);
    }
  };

  // Group settlement
  const handleGroupSettlement = async (data: SettlementFormData, selectedUserId: string) => {
    if (!selectedUserId) {
    ...
    const amount = parseFloat(data.amount);
    const groupData = entityData as GroupEntityData;
    ...
    try {
      ...
      const paidByUserId =
        data.paymentType === "youPaid" ? currentUser!._id : selectedUser.userId as Id<"users">;

      const receivedByUserId =
        data.paymentType === "youPaid" ? selectedUser.userId as Id<"users"> : currentUser!._id;
