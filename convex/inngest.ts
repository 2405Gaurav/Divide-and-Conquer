...
export const getUsersWithOutstandingDebts = query({
  handler: async (ctx) => {
    ...
    const result: {
      ...
      name: string;
      email: string;
      debts: { userId: Id<"users">; name: string; amount: number; since: number }[];
    }[] = [];
    ...
    const userCache = new Map<string, Awaited<ReturnType<typeof ctx.db.get>>>();
    ...
    for (const user of users) {
      ...
      for (const exp of expenses) {
        ...
        if (exp.paidByUserId !== user._id) {
          ...
          const entry = ledger.get(exp.paidByUserId as string) ?? {
            amount: 0,
            since: exp.date,
          };
          entry.amount += split.amount; // user owes
          entry.since = Math.min(entry.since, exp.date);
          ledger.set(exp.paidByUserId as string, entry);
        }
        ...
        // Case B: user paid, others appear in splits
          ...
          for (const s of exp.splits) {
            ...
            const entry = ledger.get(s.userId as string) ?? {
              amount: 0,
              since: exp.date,
            };
            entry.amount -= s.amount; // others owe user
            ledger.set(s.userId as string, entry);
          }
        }
      }
      ...
      for (const st of settlements) {
        ...
        if (st.paidByUserId === user._id) {
          const entry = ledger.get(st.receivedByUserId as string);
          if (entry) {
            entry.amount -= st.amount;
            if (entry.amount === 0) ledger.delete(st.receivedByUserId as string);
            else ledger.set(st.receivedByUserId as string, entry);
          }
        }
        // Someone paid the user → reduce negative balance (they owed user)
        else if (st.receivedByUserId === user._id) {
          const entry = ledger.get(st.paidByUserId as string);
          if (entry) {
            entry.amount += st.amount; // entry.amount is negative
            if (entry.amount === 0) ledger.delete(st.paidByUserId as string);
            else ledger.set(st.paidByUserId as string, entry);
          }
        }
      }

      /* ── 3) build debts[] list with only POSITIVE balances ──────────── */
      const debts: { userId: Id<"users">; name: string; amount: number; since: number }[] = [];
      for (const [counterId, { amount, since }] of ledger) {
        if (amount > 0) {
          const counter = await getUser(counterId as Id<"users">);
          debts.push({
            userId: counterId as Id<"users">,
...
export const getUsersWithExpenses = query({
  handler: async (ctx) => {
    const users = await ctx.db.query("users").collect();
    const result: { _id: Id<"users">; name: string; email: string }[] = [];
