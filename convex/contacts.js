...
//CLERK SESSION is running in the fronted and that gives this function hte JWT token which is used to identify the user and then we can use that to fetch the user from the database and then we can use that user to fetch the contacts and groups that the user is a part of. This way we don't have to worry about authentication in this function and we can just focus on fetching the data that we need.
    ...
    // If someone else paid → add them
    // For each split → add everyone except yourself--->more cotactsssss
