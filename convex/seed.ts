...
type GroupDoc = Doc<"groups"> & { _id: Id<"groups"> };
type ExpenseDoc = Doc<"expenses"> & { _id: Id<"expenses"> };
...
async function createGroups(ctx: MutationCtx, users: UserDoc[]): Promise<GroupDoc[]> {
  const now = Date.now();
...
async function createOneOnOneExpenses(ctx: MutationCtx, users: UserDoc[]): Promise<ExpenseDoc[]> {
  const now = Date.now();
...
async function createSettlements(
  ...
  users: UserDoc[],
  groups: GroupDoc[],
  oneOnOneExpenses: ExpenseDoc[],
  groupExpenses: ExpenseDoc[]
) {
  const now = Date.now();
  ...
  const settlementIds: Id<"settlements">[] = [];
